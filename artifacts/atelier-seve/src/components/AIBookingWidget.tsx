...
export function AIBookingWidget() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: ChatMessageRole.assistant, content: "Benvenuta in Atelier Sève! Sono Chiara, la tua consulente di bellezza. Come posso aiutarti oggi?" }
  ]);
  ...
  const handleSend = () => {
    ...
    sendChat.mutate({ data: { messages: newMessages } }, {
      onSuccess: (response) => {
        ...
        // Simple voice output if available
  ...
  const toggleListen = () => {
    ...
    if (isListening) {
      ...
      return; // The actual stopping will be handled by the recognition instance if stored
    }
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-gradient-to-tr from-secondary to-border shadow-[0_0_20px_rgba(201,160,110,0.4)] flex items-center justify-center z-40 transition-transform hover:scale-105 group"
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-border/20 bg-foreground/50 backdrop-blur-sm z-10">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-serif italic text-xl text-background">Chiara — Consulente AI</h3>
                </div>
                <button 
                  onClick={() => setIsOpen(false)}
                  className="text-muted-foreground hover:text-background transition-colors p-2"
              ...
              <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 scrollbar-hide">
                {messages.map((msg, i) => (
                  <div 
                    key={i} 
                    className={`flex flex-col max-w-[85%] ${msg.role === ChatMessageRole.user ? "self-end items-end" : "self-start items-start"}`}
                  >
                    <div 
                    ...
                    {msg.content.includes("confermata") && msg.role === ChatMessageRole.assistant && (
                      <div className="mt-3 px-4 py-3 border border-border rounded-lg bg-background/5 text-background font-sans text-sm flex items-center gap-2">
                        <span className="text-border">✓</span> Prenotazione confermata
                      </div>
                    )}
                  </div>
                ))}
