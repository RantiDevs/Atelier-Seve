...
function Slider({ title }: { title: string }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      >
        {/* After Image (Background) - Cream with gold tint */}
        <div className="absolute inset-0 bg-gradient-to-br from-background to-[#f0e6d2]">
          ...
          <span className="absolute top-4 right-4 font-sans text-xs uppercase tracking-widest text-border font-bold">Dopo</span>
        </div>

        {/* Before Image (Foreground overlay) - Warm gray-mauve */}
        <div 
          className="absolute inset-0 bg-gradient-to-br from-[#d4cdc5] to-muted border-r border-border"
          style={{ clipPath: `inset(0 ${100 - sliderPosition}% 0 0)` }}
        >
          <span className="absolute top-4 left-4 font-sans text-xs uppercase tracking-widest text-foreground font-bold">Prima</span>
        </div>
  ...
  return (
    <section className="w-full bg-background py-24 px-6 md:px-12 lg:px-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <Slider title="Laminazione Ciglia" />
        <Slider title="Pulizia Viso Profonda" />
        <Slider title="Trattamento Labbra" />
      </div>
    </section>
  );
}
